    ...
    void add_buttons()
    {
        ...
        btn_edit->events().click(
            [&]() {
                ...
                takenhanced_dialog keyBindingForm(kb.command, kb.keyBinding, "Hit any Key...", nana::API::make_center(160, 110));
...
public:
    tab_page_keys(nana::window parent) : nana::panel<false>(parent)
    {
        ...
        layout->div("margin=15                            \
                     vert <weight=20 searchBox>           \
                          <<margin=[10] keyBindingsList><weight=100 vert margin=[10, 20, 0, 20] <weight=80 vert gap=5 actionButtons>>>     ");
